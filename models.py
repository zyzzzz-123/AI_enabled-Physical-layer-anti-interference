...
import channels
import torch
from channels import plot_fft_one_channel_2, channel_choose, encoded_normalize, ob_to_one_hot
...
class ob_autoencoder(nn.Module):
    ...
    def forward(self, x, interf_500, interf_500_2, interf_64, step_total, device):
        ...
        ob_one_hot1 = ob_to_one_hot(ob_result, device)

        ob_result2  = self.ob_decoder(interf_500_2)
        ob_one_hot2 = ob_to_one_hot(ob_result2, device)

        ## encoder
        encode_input = torch.cat([ob_one_hot1,x],dim=1)
        ...
        encoded_choose = channel_choose(encoded_normalized,self.args, device)  # encoded_choose shape: (bs, 64, 2).
        ...
        ## decoder
        decoder_input =  torch.cat([transmitted[:,:,0],transmitted[:,:,1],ob_one_hot2],dim=1)
